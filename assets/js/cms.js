// ============================================================
// Horizon Gate School — CMS content loader
...
(function () {
  const API_BASE = window.HORIZON_API_BASE || '';
  ...
  function newsCardHTML(n) {
    ...
    return `
      <article data-gallery-item data-category="${esc(cat)}" class="group hg-photo-card rounded-sm">
  ...
  function teacherCardHTML(t) {
    ...
    return `
      <div class="shrink-0 w-52 snap-start">
        <div class="hg-frame hg-frame-dark rounded-sm mb-4"><img src="${esc(img)}" alt="${esc(t.name)}" loading="lazy" class="hg-frame-img w-full aspect-[4/5] object-cover"></div>
        <p class="font-display text-warmWhite text-sm text-center">${esc(t.name)}</p>
        <p class="text-xs text-goldBright mt-1 text-center">${esc(t.role)}</p>
        ${t.qualifications ? `<p class="text-xs text-warmWhite/45 mt-1 text-center">${esc(t.qualifications)}</p>` : ''}
      </div>`;
  }
